    ...
    while true do
      ...
      if(IsNearBank() or IsNearATM()) then
        if (atBank == false) then
          TriggerEvent('es_rp:notify', "Drücke E um auf dein Konto zuzugreifen")
        end
        atBank = true
        if IsControlJustPressed(1, 51)  then -- IF INPUT_PICKUP Is pressed
          if (IsInVehicle()) then
            TriggerEvent('chatMessage', "", {255, 0, 0}, "^1Du kannst die Bank nicht in einem Fahrzeug benutzen!");
          else
            if bankOpen then
              closeGui()
              bankOpen = false
...
AddEventHandler('bank:deposit', function(amount)
  if(IsNearBank() == true or depositAtATM == true and IsNearATM() == true or depositAnywhere == true ) then
    if (IsInVehicle()) then
      TriggerEvent('chatMessage', "", {255, 0, 0}, "^1Du kannst den Bankautomat nicht in einem Fahrzeug benutzen!");
    else
      TriggerServerEvent("bank:deposit", tonumber(amount))
    end
  else
    TriggerEvent('chatMessage', "", {255, 0, 0}, "^1Du kannst nur in einer Bank Geld einzahlen!");
  end
end)
...
AddEventHandler('bank:withdraw', function(amount)
  if(IsNearATM() == true or IsNearBank() == true or withdrawAnywhere == true) then
    ...
    TriggerEvent('chatMessage', "", {255, 0, 0}, "^1Das ist weder eine Bank, noch ein Bankautomat!");
  end
end)
...
AddEventHandler('bank:givecash', function(toPlayer, amount)
  if(IsNearPlayer(toPlayer) == true or giveCashAnywhere == true) then
    ...
    if (playing ~= false) then
      ...
      TriggerEvent('chatMessage', "", {255, 0, 0}, "^1Dieser Spieler ist nicht online!");
    end
  else
    TriggerEvent('chatMessage', "", {255, 0, 0}, "^1Dieser Spieler ist nicht in deiner Nähe!");
  end
end)
