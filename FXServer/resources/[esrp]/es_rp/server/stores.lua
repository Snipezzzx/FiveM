		...
		if weaponsOwned[source][w] == nil then
			TriggerEvent('es:getPlayerFromId', source, function(user)
				if user.getMoney() >= WEAPON_PRICES[w] then
					...
					TriggerClientEvent('es_rp:notify', source, 'Vielen Dank für ihren Einkauf!', "CHAR_MILSITE", "Warstock Cache & Carry", "Bestellung")
					TriggerClientEvent('es_rp:giveWeapon', source, w)
					weaponsOwned[source][w] = true
				else
					TriggerClientEvent('es_rp:notify', source, '~r~Du hast nicht genug Bargeld')
				end
			end)
		else
			TriggerClientEvent('es_rp:notify', source, '~r~Diese Waffe besitzt du bereits')
		end
	end
end)