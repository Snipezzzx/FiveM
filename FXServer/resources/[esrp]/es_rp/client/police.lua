...
Citizen.CreateThread(function()
	for i,v in pairs(arrestPoints)do
		...
		AddTextComponentSubstringPlayerName("Gefängnis")
	...
	while true do
		...
		if JAILED then
			if GetVehiclePedIsTryingToEnter(GetPlayerPed(-1)) > 0 then
				ClearPedTasksImmediately(GetPlayerPed(-1))
				TriggerEvent('es_rp:notify', "~r~Du kannst nicht in ein Fahrzeug einsteigen")
			end
			if Vdist(1679.28, 2629.77, 64.45, pPos.x, pPos.y, pPos.z) > 200.0 then
				SetEntityCoords(GetPlayerPed(-1), 1679.28, 2629.77, 45.45)
				TriggerEvent('es_rp:notify', "~r~Du hast dich zu weit entfernt und wurdest zurück teleportiert")
			end
		end
